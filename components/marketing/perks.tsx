...
const Perks = () => {
  return (
    <section className="flex flex-col items-center justify-center py-16 w-full bg-white">
      <h2 className="text-4xl md:text-5xl font-extrabold text-center mb-2 text-gray-900">Launch your next</h2>
      <h3 className="text-4xl md:text-5xl font-extrabold text-center mb-10 text-gray-900">Webinar</h3>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-5xl">
        {PERKS_CARDS.map((perk, idx) => (
          <div
            key={perk.title}
            className={`rounded-2xl shadow-lg p-6 flex flex-col items-center ${perk.bg}`}
          >
            <div className="w-full flex justify-center mb-4">
              <div className="relative w-56 h-40 rounded-xl overflow-hidden bg-gray-50">
                <Image src={perk.image} alt={perk.title} fill className="object-cover" />
              </div>
            </div>
            <h4 className="text-xl font-extrabold text-gray-900 mb-2 text-center">{perk.title} <span className="inline-block align-middle">→</span></h4>
            <p className="text-gray-700 text-base text-center">{perk.description}</p>
          </div>
        ))}
      </div>
    </section>
