  ...
  return (
    <section className="w-full py-20 bg-[#F8FAFC]">
      <div className="max-w-6xl mx-auto px-4">
        <h2 className="text-5xl font-extrabold text-center text-gray-900 mb-4">Get inspired with our blog</h2>
        <p className="text-center text-lg text-gray-500 mb-12">Get a headstart on your journey with our latest updates, tips and features</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          {blogPosts.map((post, idx) => (
            <div key={idx} className={`rounded-2xl p-6 flex flex-col items-start ${post.bg}`}>
              <div className="w-full flex justify-center mb-4">
                <div className="relative w-48 h-48 rounded-full overflow-hidden bg-gray-50">
                  <Image src={post.image} alt={post.title} fill className="object-cover" />
                </div>
              </div>
              <h3 className="text-xl font-extrabold text-gray-900 mb-2 leading-snug">{post.title}</h3>
